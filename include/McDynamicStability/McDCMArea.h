...
class McDCMArea
/*! \brief We calculate the `Multi-contact DCM-area` using the intersection of the `Multi-contat ZMP are` and the
 * `Multi-contact Com static equilibrium` area
 */
{
...
public:
  ...
  void print() const
  {
    std::cerr << red << "McDCMArea has: " << getNumVertex() << " vertices (maximum: " << getMaxNumVertex() << reset
              << std::endl;
  }
